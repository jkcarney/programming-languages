...
let rec build2 (rand,depth) = 
  if depth == 0 then
    ...
    | 7 -> buildMultiply3(build2 (rand,depth - 1), build2 (rand,depth - 1), build2 (rand,depth - 1))
    | 8 -> buildAbsSqrt(build2 (rand,depth - 1))
    | _ -> buildSine(build2 (rand,depth - 1))
;;
